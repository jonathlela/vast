...
#define MOVEMENT_SECTION 2
	...
	{
		__pos_list = new map<int, vector<Position> >;
	}
								  ...
								  int num_nodes, int num_steps, double speed)
	{
		...
		map<int, vector<Position> > &_pos_list = *__pos_list;
		...
		if (replay == true)
		{
			for (int p = 0; p < num_nodes; p++)
			{
				...
				if (record->read (MOVEMENT_SECTION, &_pos_list[p][0], sizeof (Position) * (num_steps + 1), 1) != 1)
				{
		...
		else
		{
			...
			if (record != NULL)
			{
				for (int p = 0; p < num_nodes; p++)
				{
					if (record->write (MOVEMENT_SECTION, &_pos_list[p][0], sizeof (Position) * (num_steps + 1), 1) != 1)
					{
