...
public:
	...
	SimNode (int id, MovementGenerator *move_model, SimPara &para, bool as_relay, id_t beh_id);
	...
	// for behavior model 
	void       setBehId(const id_t bid);
	inline     id_t getBehId();
	void       setJoinStep (const int s);
	inline int getJoinStep();
...
private:
    ...
    map<id_t, Node *>   _neighbors;     // known neighbors
