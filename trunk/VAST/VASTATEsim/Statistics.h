...
#ifndef VASTATESIM_STATISTICS_H
#define VASTATESIM_STATISTICS_H
...
public:
    ...
    void print_stat ()
    {
        ...
        fprintf (_fp, "[ avg] sent: %7u recv %7u\n", (size_t)((float)send_accumulated / (float)_simnodes.size () / (float)_steps * _para.STEPS_PERSEC), (size_t)((float)recv_accumulated / (float)_simnodes.size () / (float)_steps * _para.STEPS_PERSEC));
		...
		for (i=0; i<_simnodes.size (); i++)
        {
            SimPeer *node = _simnodes[i];

            fprintf (_fp, "[%4ld] %10u %10u\t\n", node->get_id (), node->accumulated_send (), node->accumulated_recv ());
        }
        ...
        fprintf (_fp, "\nInconsistent nodes\n");
        size_t n = _inconsistent_nodes.size ();
