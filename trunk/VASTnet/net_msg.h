...
namespace Vast {
    ...
    {
    public:
        netmsg (id_t src, char const *m, size_t s, timestamp_t ts, netmsg *p = NULL, netmsg *n = NULL) //, msgtype_t t, id_t id = 0)
            :from (src), msg (NULL), time (ts), size (s) //, msgtype(t), msg_id (id)
        {
            if (p == NULL)
                p = this;
            if (n == NULL)
