...
int InitVASTsim (VASTsimPara &para)
{
    ...
    // create vast factory
    g_world = new vastverse (g_para.NET_MODEL, g_para.CONNECT_LIMIT, g_para.LOSS_RATE, g_para.FAIL_RATE, g_para.UPLOAD_LIMIT, g_para.DOWNLOAD_LIMIT);
...
int CreateNode ()
{
    ...
    while (1)
    {
        ...
        if (g_nodes[i]->vnode->isJoined () == true)
            break;        
    }
...
{   
    return g_nodes[index]->vnode->getManagers ();
}
