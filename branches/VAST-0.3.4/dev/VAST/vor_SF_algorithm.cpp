...
Site* 
sfvoronoi::
intersect(Halfedge *el1, Halfedge *el2 , Point *p)
{
    Edge *e1,*e2, *e;
    Halfedge *el;
    double d, xint, yint;
...
void sfvoronoi::calsfv()
{
    ...
    {        
        // free all memory
        for (int i=0; i< (int)buf_list.size (); i++)
            free (buf_list[i]);
            //delete[] buf_list[i];

        buf_list.clear ();
        
        // allocate a continous piece of memory                
        //sfvBuffer = new char[mem_alloc];
...
bool sfvoronoi::collides (int index, const point2d &center, int radius)
{
         ...
         it != mSites[index].mEdgeIndexSet.end (); it++)
    {
        ...
        for (int k=0; k<2; k++)
        {
            ...
            {
                if (mVertex[idx].dist (center) <= (double)radius) 
                {
...
bool sfvoronoi::enclosed (int index, point2d& center, int radius)
{
         ...
         it != mSites[index].mEdgeIndexSet.end(); it++)
    {
        ...
        if ( idx1 == -1 || 
             idx2 == -1 ||
             center.dist (mVertex[idx1]) >= (double)radius ||
             center.dist (mVertex[idx2]) >= (double)radius ) 
        {
            return false;
        }
    }
