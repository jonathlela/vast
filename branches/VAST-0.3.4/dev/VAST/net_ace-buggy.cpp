...
namespace VAST {
    ...
    connect (id_t target_id, Addr addr)
    {
        ...
        {
            // NOTE handler will be released automatically
            handler->close();
            return (-1);
        }
    ...
    {
#ifdef DEBUG_DETAIL
        //printf ("%4d [%3d]    sendmsg   to: %3d type: %-8s size:%3d\n", time, (int)_id, target, VAST_DC_MESSAGE[msgtype], len);
#endif  
        // if it's a loop-back message we simply store it
        if (target == _id)
        {
            return storemsg (target, msgtype, msg, len, time);
        }
        ...
        // storage buffer
        static char buf[VAST_BUFSIZ + 1];
                
        //
        // prepare bytestring
        //

        // append id, message type & timestamp in front of the bytestring
        char *p = buf + sizeof (short int);
        memcpy (p, &_id, sizeof (id_t));
        p += sizeof (id_t);

        memcpy (p, &msgtype, sizeof (msgtype_t));
        p += sizeof (msgtype_t);

        memcpy (p, &time, sizeof (timestamp_t));
        p += sizeof (timestamp_t);

        memcpy (p, msg, len);
        
        // first byte is size
        short int size = len + sizeof (id_t) + sizeof (msgtype_t) + sizeof (timestamp_t);

        // put size in front
        memcpy (buf, &size, sizeof (short int));
        ...
        {    
            ...
            {
                // create new message
                netmsg *newmsg = new netmsg (_id, buf, size, msgtype, time, NULL, NULL);
                ...
                if (_msgqueue_out.find (target) != _msgqueue_out.end ())            
                {
                    ...
                    out_queue->append (newmsg, time);
                    _msgqueue_out_length[target]  = _msgqueue_out_length[target] + size;
                }
                else
                {
                    _msgqueue_out[target] = newmsg;
                    _msgqueue_out_length[target] = size;
                }                 
            }                 
        }
        else
        {
            ...
            ACE_INET_Addr target_addr ((u_short)addr.publicIP.port, (ACE_UINT32)addr.publicIP.host);
            _udp->send (buf, size, target_addr);
        }
    ...
    {
        // message buffer (hopefully it's enough :)
        static char buf[VAST_BUFSIZ];
        ...
        int size = curr_msg->size;
        memcpy (buf, curr_msg->msg, size);
        *msg = buf;
    ...
    flush ()
    {
        ...
        while (it != _msgqueue_out.end ())
        {
            ...
            // TODO: a better way than to allocate memory every time?
            int len = _msgqueue_out_length[target];
    ...
    unregister_conn (id_t id)
    {
        ...
        storemsg (id, 0, msg, 1+sizeof (id_t), _curr_time);
