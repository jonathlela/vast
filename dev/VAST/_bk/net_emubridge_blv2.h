...
/*
 * net_emubridge_blv2.h -- a shared class between various net_emu classes
 *                       for discovering pointers of other net_emu
 *                       to send messages
 *                       bandwidth limited version
 */

#ifndef VAST_NET_EMUBRIDGE_BLV2_H
#define VAST_NET_EMUBRIDGE_BLV2_H

#include "net_emubridge.h"

namespace VAST {

    class net_emubridge_blv2 : public net_emubridge
    {
    public:
        net_emubridge_blv2 (int loss_rate, int fail_rate, int seed = 0, timestamp_t init_time = 1, size_t upload_quota = 0, size_t download_quota = 0)
            : net_emubridge (loss_rate, fail_rate, seed, init_time), default_upcap (upload_quota), default_downcap (download_quota)
        {
        }
        ...
        void register_id (id_t temp_id, id_t id);

        void release_id (id_t id);

        // Return remaining send/recv quota for the node #node_id
        unsigned int get_quota (bandwidth_type_t type, id_t node_id);

        // Decrease quota spent after some success allocation or transmission
        bool spend_quota (bandwidth_type_t type, id_t node_id, size_t spent_quota);

        // Set default quota of node
        bool set_default_quota (id_t node_id, bandwidth_type_t type, size_t quota);
...
#endif // VAST_NET_EMUBRIDGE_BLV2_H
